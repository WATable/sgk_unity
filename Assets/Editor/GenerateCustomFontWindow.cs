...
public class GenerateCustomFontWindow : Editor
{
//本方法是通过裁切的sprite导出字体文件，裁切使用的是unity自带的sprite editor，方便操作。  
	...
	static void CreateMyFontSprite(){  
		...
		Object [] sprites_ = AssetDatabase.LoadAllAssetsAtPath(selectionPath);

		List<Sprite> ss = new List<Sprite>();
		for (int i = 0; i < sprites_.Length; i++) {
			if (sprites_[i].GetType() == typeof(Sprite)) {
				ss.Add((Sprite)sprites_[i]);
			}
		}
		...
		for(int i=0;i<sprites.Length;i++){  
            ...
            string [] names = spr.name.Split('_');
            if (names[names.Length -1] .Length == 1) {
                info.index = (int)names[names.Length - 1][0];
            } else {
                info.index = System.Int32.Parse(names[names.Length - 1]);
            }
