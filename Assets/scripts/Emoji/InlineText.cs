        ...
        foreach (var item in _ListHrefInfos)
        {
            for (int i = 0; i < item.boxes.Count; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    m_TempVerts[j].color = new Color(59/255,1,188/255,0);
    ...
    private string GetOutputText()
    {
        ...
        foreach (Match match in _InputTagRegex.Matches(text))
        {
            ...
            if (_tempID <0 )
            {
                ...
                _textBuilder.Append("<color=#3BFFBCFF>");
            ...
            else
            {
                ...
                _textBuilder.Append(@"<quad size=" + (_tempGroup.size*_InlineManager.EmojiScale) + " width=" + _tempGroup.width + " />");

                SpriteTagInfo _tempSpriteTag = new SpriteTagInfo
                {
                    _ID = _tempID,
                    _Tag = _tempTag,
                    _Size = new Vector2((_tempGroup.size*_InlineManager.EmojiScale) * _tempGroup.width, (_tempGroup.size*_InlineManager.EmojiScale)),
                    _Pos = new Vector3[4],
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        foreach (var hrefInfo in _ListHrefInfos)
        {
            ...
            for (var i = 0; i < boxes.Count; ++i)
            {
                ...
                {
					if (onClick != null) {
						onClick (hrefInfo.name,hrefInfo.id);
					}
