...
function Battle:API_ShowRandomBuff(attacker, ...)
    self:Dispatch("ShowRandomBuff", ...)
end

function Battle:InitAiProperty(level, id, cfg, property_list)
    local ai_suit, ai_titles = battle_config.LoadAiNpcCfg(level, id)
    if not next(ai_suit) or not next(ai_suit.suit_rand_pool) then
        return
    end
...
function Battle:API_GetSkillEffectCfg(attacker, id)
