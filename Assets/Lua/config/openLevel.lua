...
local function checkEvent(cfg)
    ...
    for i = 1, 1 do
        ...
        elseif cfg["event_type"..i] == 5 then
            ...
            _staus = (date.wday == 0 or date.wday == 6);
        end
    end
...
local function GetCloseInfo(id, heroLevel)
    ...
    if _cfg then
        ...
        for i = 1, 1 do
            ...
            elseif _cfg["event_type"..i] == 5 then
                return SGK.Localize:getInstance():getValue("cailijingjichang_tips_01")
            end
        end
    else
        ERROR_LOG("_cfg is nil,id",id)
    end
    return ""
end
    ...
    if suits and suits > 0 then
        ...
        if _suitsCfg then
            if _suitsCfg.open_lev > module.HeroModule.GetManager():Get(11000).level then
                return false, _suitsCfg.open_lev,"shortLv"--SGK.Localize:getInstance():getValue("tips_lv_02", _suitsCfg.open_lev)
            end
        else
            return false, _suitsCfg.open_lev--SGK.Localize:getInstance():getValue("tips_lv_02", _suitsCfg.open_lev)
        end
    end
    -- if place then
