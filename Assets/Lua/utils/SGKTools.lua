	...
	if is_open then
		...
		if heroInfo then
			...
			if showDetail then
				...
				SGK.ResourcesManager.LoadAsync(animation, "roles/".._mode.."/".._mode.."_SkeletonData",function (resource)
					DispatchEvent("stop_automationBtn",{automation = false,mandatory = false})
...
function SGKTools.loadEffectVec3(name,Vec3,time,lock,fun,scale)--加载一个全屏的UI特效
	local eff = GetUIParent(SGK.ResourcesManager.Load("prefabs/effect/"..name),UnityEngine.GameObject.FindWithTag("UGUITopRoot"))
...
function SGKTools.showScrollingMarquee(desc,level)
	...
	if ScrollingMarqueeView == nil then
		...
		ScrollingMarqueeView = CS.SGK.UIReference.Setup(GetUIParent(SGK.ResourcesManager.Load("prefabs/ScrollingMarquee"),parent))
...
function SGKTools.SwitchTeamTarget( group )
	...
	if _status then
		...
		if group == 0 then
			return showDlgError(nil,SGK.Localize:getInstance():getValue("zudui_fuben_07"))
...
function SGKTools.CheckPlayerAfkStatus( pid )
	...
	if utils.SGKTools.GetTeamState() and module.TeamModule.GetAfkStatus(pid) then
		return true; 
	end
end
...
--历练任务共享Tip
function SGKTools.ShareTaskTipShow(data)
	local tempObj = SGK.ResourcesManager.Load("prefabs/Tips/shareTaskTip")
	local obj = nil;
	local UIRoot = UnityEngine.GameObject.FindWithTag("UITopRoot")
	if UIRoot then
		obj = CS.UnityEngine.GameObject.Instantiate(tempObj,UIRoot.gameObject.transform)
	...
	local TipsRoot = CS.SGK.UIReference.Setup(obj)
	local _taskItem = TipsRoot.view.shareTask

	local _questCfg = data
	--任务品质
	...
	--怪物形象
	local mode = _questCfg.condition[1].id
		...
		SGK.ResourcesManager.LoadAsync(bossAnim, string.format("roles/%s/%s_SkeletonData", mode, mode), function(o)
			if o ~= nil then
				...
				SGK.ResourcesManager.LoadAsync(bossAnim, string.format("roles/11000/11000_SkeletonData"), function(o)
					bossAnim.skeletonDataAsset = o
					local _pos, _scale = DATABASE.GetBattlefieldCharacterTransform(tostring(11000), "ui")
					if _pos and _scale then
						bossAnim.transform.localPosition =  _pos+Vector3(0,-10,0)
						bossAnim.transform.localScale = _scale
					end
	...
	--是否为共享任务
	local _share = _questCfg.can_share == 1
	_taskItem.questInfo.name.Image:SetActive(_share)
	...
	for i=1,#_questCfg.reward do
		if _questCfg.reward[i].type ~= 0 and _questCfg.reward[i].id ~= 0 and _questCfg.reward[i].value ~= 0 then
			local itemCfg = utils.ItemHelper.Get(_questCfg.reward[i].type,_questCfg.reward[i].id,nil,_questCfg.reward[i].value)
			if itemCfg and _taskItem.questInfo.rewards[i] then
				local rewardItem = _taskItem.questInfo.rewards[i]
				rewardItem:SetActive(true)
				rewardItem[SGK.LuaBehaviour]:Call("Create", {customCfg = itemCfg})
			end
		end
	end

	--任务条件
	local _questInfo = module.QuestModule.Get(_questCfg.id)
		local _targetCount = _questCfg.condition[1].count     --_questInfo.condition[1].count
		local _finishedCount = 0--_questInfo.records

	for i=1,_taskItem.questInfo.conditions.transform.childCount do
		_taskItem.questInfo.conditions[i].Text[UI.Text].text = _questCfg.desc
		
		_taskItem.questInfo.conditions[i]:SetActive(true)
		_taskItem.questInfo.conditions[i].num[UI.Text].text = string.format("%s/%s",_finishedCount,_targetCount)
		_taskItem.questInfo.conditions[i].progress[UI.Image].fillAmount = _finishedCount/_targetCount
