...
local function EventManager_Dispatch(manager, event, ...)
    if event and (manager.watcher[event] or manager.watcher['*'] or manager.cos[event]) then
        if manager.name then
            print("[EventManager:" .. manager.name .. "] dispatch", event, ...);
        end
...
local function EventManager_Call(manager, event, ...)
    if manager.name then
        print("[EventManager:" .. manager.name .. "] call", event, ...);
    end
...
local function EventManager_Tick(manager)
    --[[
    if manager.tick_lock then
        return;
    end
    manager.tick_lock = true;
    --]]

    local event_list = manager.list;
    manager.list = {};

    local i = 0
    while i < #event_list do
        i = i + 1;
        EventManager_Call(manager, safe_unpack(event_list[i]));
    end

    -- manager.tick_lock = nil;
end
